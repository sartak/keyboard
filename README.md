...
To help burn the chords into muscle memory, there's an optional mode that forces usage of studied chords. The keyboard watches for each word that is typed reasonably quickly. If such a word matches a studied chord (defined by having a non-suspended Anki card for it), then the keyboard will instantly delete that word. That indicates the chord should be used instead. If the chord isn't quite committed to memory, it'd be a bother to have to go look up its combo. So immediately after such a chord-deletion, the Tab key changes behavior to emit the chord's combo right into the text field. Another press of Tab (or using the delete-chord combo) will erase the combo.
...
- `f` + `i` + `n` + `⨧` → final fantasy (shifted: Final Fantasy)
